...
if __name__ == '__main__':
    work_book_path = input('请输入目标工作簿路径:')
    # 打开目标工作簿
    workbook = load_workbook(work_book_path)
    # 获取目标工作表
    worksheet = workbook[input('请输入数据源工作表名:')]
    # 新建工作表存储数据
    new_work_sheet = workbook.create_sheet(input('请输入要存储处理后数据工作表名:'))
    # 新建工作表添加表头
    new_work_sheet.append(('Customer-PN', 'Customer-describe', "Q'ty", 'Location', 'Vendor', 'Customer-MPN'))
    # 获取数据
    data_list = worksheet.iter_rows(min_row=2, values_only=True)
    for data in data_list:
        # 判断是否有厂商数据
        if data[4] == '-' or not data[4]:
            new_work_sheet.append(data)
        else:
            # 拆分厂商数据为列表
            vendor_data_list = data[4].split('\n')
            # 循环遍历每一个厂商的数据
            for vendor_data in vendor_data_list:
                ...
                try:
                    ...
                    if num == 1:
                        ...
                        new_work_sheet.append((data[0], data[1], data[2], data[3], vendor, MPN))
                        # else:
                        ...
                        # 添加数据到工作表
                        # if vendor_data == vendor_data_list[0]:
                        new_work_sheet.append((data[0], data[1], data[2], data[3], vendor1, MPN1))
                        new_work_sheet.append((data[0], data[1], data[2], data[3], vendor2, MPN2))
                        # else:
